...
def restricted(func):
    ...
    def wrapped(bot, update, *args, **kwargs):
        ...
        ok = False
        list_of_users = DatabaseHandler("resources/datastore.db").get_all_users()
        for registered_user in list_of_users:
            if user_id == registered_user:
                ok = True
                print("iteration")
        if not ok:
            print("unauthorized access denied for {}.".format(user_id))
            msg = "This is an open source software, which means you can have your own!"
